...
public final class AdminPagingHelper {
     ...
     */
    public static int clampPage(int page, long totalCount, int size) {
    ...
    public static int calculateTotalPages(long totalCount, int size) {
        ...
        int safeSize = Math.max(size, 1);
        // 덧셈 오버플로를 피하기 위해 ceil(totalCount / size)를 아래 형태로 계산합니다.
        long totalPages = 1L + ((totalCount - 1L) / safeSize);
